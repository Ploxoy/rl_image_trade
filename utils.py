...
def get_RSI(df, col_name, intervals):
    ...
    """
    ind_name="RSI"
    new_df = pd.DataFrame(df[col_name].copy())
    for i in intervals:
        new_df['{}_{}'.format(ind_name,i)] = rsi(new_df[col_name], i, fillna=True )
    
    new_df=new_df.drop(col_name,axis= 1)/100.0
    print("RSI done")
    return new_df
...
def get_williamR(df, col_name, intervals):
    ...
    for i in tqdm(intervals):
        ...
        new_df["WR_" + str(i)] = wr(df['high'], df['low'], df['close'], i, fillna=True)

    print_time("Calculation of WilliamR Done", stime)
    return new_df.drop(col_name,axis= 1)/100.0
...
def get_mfi(df, intervals):
    ...
    new_df = pd.DataFrame(df[col_name].copy())
    for i in tqdm(intervals):
        new_df['MFI_' + str(i)] = money_flow_index(df['high'], df['low'], df['close'], df['vol'], n=i, fillna=True)
...
def get_SMA(df, col_name, intervals):
    ...
    new_df = pd.DataFrame(df[col_name].copy())
    for i in tqdm(intervals):
        new_df[col_name + '_sma_' + str(i)] = df_ss[col_name + '_' + str(i) + '_sma']
...
def get_EMA(df, col_name, intervals):
    ...
    new_df = pd.DataFrame(df[col_name].copy())
    for i in intervals:
        new_df['{}_{}'.format(ind_name,i)] = ema_indicator(df[col_name], i, fillna=True )
    
    new_df=new_df.drop(col_name,axis= 1)
    print("{} done".format(ind_name))
    return new_df
...
def get_CMF(df, col_name, intervals):
    ...
    for i in tqdm(intervals):
        df['cmf_' + str(i)] = chaikin_money_flow(df['high'], df['low'], df['close'], df['vol'], i, fillna=True)
...
def get_EOM(df, col_name, intervals):
    ...
    new_df = pd.DataFrame(df[col_name].copy())
    for i in tqdm(intervals):
        new_df['eom_' + str(i)] = ease_of_movement(df['high'], df['low'], df['vol'], n=i, fillna=True)
    new_df=new_df.drop(col_name,axis= 1)
...
def make_observations(data, periods, col ="close", ts=12):
    ...
    stats.append(get_EOM(data,col, periods).values )
